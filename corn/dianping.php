...
function getShop($param) {
    ...
    if (!empty($shopJson) && !empty($category) && !empty($tel)) {
        ...
        $shop = array(
            ...
            //'image'     => getImage($shopJson['msg']['shopInfo']['defaultPic'],$param['shopid']),
            ...
            'city'      => '101',
            'dist'      => $dist_id,
            'street'    => getStreetId($dist_id,$param['street']),
            ...
            'content'   => "营业时间：".$shopJson['msg']['shopInfo']['businessHours']."<br>".
                           "公交线路：".$shopJson['msg']['shopInfo']['publicTransit']."<br>".
                           $content,
            'status'    => '1',
        ...
        $tmp_shop = db_select("shop")
                        ->fields("shop")
                        ->condition("name",$shop['name'])
                        ->condition("city","101")
                        ->condition("dist",$shop['dist'])
                        ->condition("street",$shop['street'])
                        ->execute()
                        ->fetch();
        if (!empty($tmp_shop)) {
            db_update("shop")
...
function getImage($url,$shop_id) {
    ...
    $fopen = @fopen($url,"rb");
    $dirname = "/cache/upload/images/dianping/".time()."_".$shop_id.".jpg";
    $saveFile = @fopen("/var/www/html/diandan".$dirname,"wb");
    if ($fopen) {
        while (!feof($fopen)) {          
