...
entity_register('book', array(
    'controller' => 'Entity\\Book\\BookEntityController',
    'primaryKey' => 'id',
    'baseTable'  => 'book',
));
...
class Book {
    ...
    public static function load($request) {
        ...
        $data = entity_load('book', array($id));
        return reset($data);
    }
    ...
    public static function loadMulti($request) {
        ...
        $data = entity_load('book', $ids);
        return $data;
    }
    
    //新增
    public static function insert($request) {
        $book = (object) $request->getParameters();
        unset($book->id);
        $book->status = 1;
        $book->created = $book->updated = time();
        entity_insert('book', $book);            
        logger()->info("新增预订成功: ".var_export((array)$book,true));
        return $book;
    }
